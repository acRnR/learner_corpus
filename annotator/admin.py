...
class LearnerCorpusAdminSite(AdminSite):
    site_header = 'Corpus of Russian Student Texts'
    site_title = 'Admin'
    index_title = 'CoRST'
...
class DocumentAdmin(admin.ModelAdmin):
    fieldsets = [
        ...
        ('Author', {'fields': [('author', 'gender', 'major', 'university'), ('student_code', 'university_code')]}),
        ('Date', {'fields': [('date1', 'date2'), ('term', 'module')]}),
        ('Text', {'fields': [('genre', 'domain', 'level')]}),
        ('Other', {'fields': [('course', 'time_limit')]}),
        ('Autocompletion', {'fields': [('annotated', 'checked')], 'classes': [('collapse')]}),
    ]

    list_display = ('title', 'author', 'gender', 'major', 'course', 'genre', 'date_displayed', 'annotated', 'checked', 'created')
    list_filter = ['gender', 'major', 'course', 'genre', 'domain', 'annotated', 'checked']
